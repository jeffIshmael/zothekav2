...
function AuthSync() {
  ...
  useEffect(() => {
    if (ready && authenticated && user && client?.account?.address) {
      const email = user.email?.address || user.google?.email;
      const name = user.google?.name || email?.split("@")[0] || "User";
      const smartWallet = client.account.address; // Actual Smart Wallet address
  ...
  }, [ready, authenticated, user, client?.account?.address]);
...
function AppShell({ children }: { children: ReactNode }) {
  ...
  }, [isAuthenticated, isLoading, isSignIn, isInvite, router]);
